...
export default function EventsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8 animate-fade-in">
      ...
      {/* Filter Bar */}
      <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800/90 rounded-2xl p-4 shadow-xl space-y-3 md:space-y-0 md:flex md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-2">
          <button className="px-4 py-2 bg-[#00A3E0] text-white text-xs font-bold rounded-xl shadow-md min-h-[40px]">
            Upcoming
          </button>
          <button className="px-4 py-2 text-slate-400 hover:text-white hover:bg-slate-800/80 text-xs font-semibold rounded-xl transition-colors min-h-[40px]">
            Past Events
          </button>
        </div>

        <div className="flex flex-wrap items-center gap-2.5">
          <select className="px-3.5 py-2.5 text-xs border border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#00A3E0] bg-slate-950/90 text-slate-200 font-medium">
          ...
          <div className="relative flex-1 sm:w-56">
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 text-xs border border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#00A3E0] text-white placeholder-slate-400 bg-slate-950/90"
            />
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="py-16 text-center text-xs text-slate-400 bg-slate-900/60 border border-slate-800/80 rounded-2xl shadow-sm">
            <div className="w-7 h-7 border-2 border-[#00A3E0] border-t-transparent rounded-full animate-spin mx-auto mb-3" />
            Loading ACM events...
          </div>
        ) : events.length === 0 ? (
          <div className="py-16 text-center text-xs sm:text-sm text-slate-400 bg-slate-900/60 border border-slate-800/80 rounded-2xl shadow-sm">
          ...
          events.map((evt) => {
            ...
            return (
              <div
                key={evt.id}
                className="group bg-slate-900/80 backdrop-blur-md border border-slate-800/80 hover:border-cyan-400/50 rounded-2xl p-5 sm:p-6 shadow-xl hover:-translate-y-1 transition-all duration-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
              >
                <div className="flex items-start sm:items-center gap-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="px-2.5 py-0.5 bg-blue-500/10 text-[#00A3E0] text-[10.5px] font-mono font-bold rounded-md border border-blue-500/20">
                        {evt.type.replace('_', ' ')}
                      </span>
                    ...
                    <h3 className="text-base font-extrabold text-white leading-snug group-hover:text-[#00A3E0] transition-colors">{evt.title}</h3>
                ...
                <div className="flex items-center justify-between sm:justify-end gap-4 pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-800">
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-6 py-3 bg-[#00A3E0] hover:bg-[#008cc0] active:scale-[0.98] text-white text-xs font-extrabold rounded-xl shadow-lg shadow-cyan-500/20 transition-all disabled:opacity-60 min-h-[44px] flex items-center justify-center"
                  >
