...
export function BrandLogos({ variant = 'navbar' }: { variant?: 'navbar' | 'hero' | 'footer' }) {
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3 lg:gap-4 py-1 flex-shrink-0">
      ...
      <Link href="/" className="flex items-center gap-2 sm:gap-2.5 group transition-opacity hover:opacity-90 flex-shrink-0">
        <div className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 flex-shrink-0 flex items-center justify-center">
          <img
            src="/images/veltech_seal.png"
        ...
        <div className="flex flex-col justify-center">
          <span className="font-extrabold text-slate-900 text-[11px] sm:text-xs md:text-[13px] lg:text-sm tracking-tight leading-tight whitespace-nowrap">
            Vel Tech High Tech
          </span>
          <span className="font-bold text-[#005596] text-[9.5px] sm:text-[10px] md:text-[11px] lg:text-xs tracking-tight leading-tight mt-0.5 whitespace-nowrap">
            ACM Student Chapter
          </span>
        </div>
      </Link>
